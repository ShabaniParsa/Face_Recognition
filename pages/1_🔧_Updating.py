...
menu = ["Adding","Deleting", "Adjusting",'Delete(StudentData)']
...
if choice == 'Delete(StudentData)':
    ...
    if submit:
        try:
            ...
            raise ValueError('Please input a int value for id.')
        id = int(id)
        try:
            DB.delete(id)
            st.success('Done!')
            inf = DB.get()
            ids = [inf[itm]['id']  for itm in inf]
            st.info(f'Less id(special):{ids.count(id)}')
        except ValueError:
            st.error("You id isnt in database.")